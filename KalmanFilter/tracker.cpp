...
void tracker::update(const DETECTIONS &detections)
{
    ...
    cout<<"_match time:"<<(end.tv_sec-start.tv_sec)*1000+(end.tv_usec-start.tv_usec) / 1000.0 <<"ms"<<endl;
#endif
    ...
    for (Track& track:tracks) {
        ...
        FEATURESS t = FEATURESS(0, FEATURE_DIMS);
        track.features = t;
    }
#ifdef PRINT_TIME
    gettimeofday(&start,NULL);
...
#ifdef PRINT_TIME
    gettimeofday(&end,NULL);
    cout<<"get partial_fit time:"<<(end.tv_sec-start.tv_sec)*1000+(end.tv_usec-start.tv_usec) / 1000.0 <<"ms"<<endl;
#endif
}
